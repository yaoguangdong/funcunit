...
public class BallPoolImage extends ImageView {
    ...
    public BallPoolImage(Context context, AttributeSet attrs) {
        ...
        //获取属性集合
  		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.BallPoolStyle);
  		textPaint = new TextPaint();
  		// 设置画笔无锯齿
  		textPaint.setAntiAlias(true);
  		// 初始化文字颜色
  		textPaint.setColor(a.getColor(R.styleable.BallPoolStyle_bpTextColor, 0xFFFEFEFE)) ;
  		
  		// 读取背景颜色
  		backgroundColor = a.getColor(R.styleable.BallPoolStyle_bpBackGroundColor, 0xFF22AA55) ;
  		
        waterPaint = new Paint();
        waterPaint.setAntiAlias(true);
        // 初始化水深颜色
  		waterPaint.setColor(a.getColor(R.styleable.BallPoolStyle_bpDraftColor, 0xFFCC4499)) ;
